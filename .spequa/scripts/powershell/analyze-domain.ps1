...
if ($env:SPEQUAFY_FEATURE) {
    $currentBranch = $env:SPEQUAFY_FEATURE
} elseif ($hasGit) {
    try {
...
if ($hasGit -and $currentBranch -notmatch '^\d{3}-') {
    $errorMsg = "Not on a feature branch. Current branch: $currentBranch. Run /spequafy first."
    if ($Json) {
...
if (-not (Test-Path $specFile)) {
    $errorMsg = "Specification file not found at $specFile. Run /spequafy first."
    if ($Json) {
        @{ error = $errorMsg } | ConvertTo-Json -Compress
    } else {
        Write-Error $errorMsg
