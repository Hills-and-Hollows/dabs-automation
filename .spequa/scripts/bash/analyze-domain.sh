...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  --data-dir      Spequafy data directory path (optional, auto-detected if not provided)"
...
if ! check_feature_branch "$CURRENT_BRANCH" "$HAS_GIT_REPO"; then
    if $JSON_MODE; then
        printf '{"error":"Not on a feature branch. Run /spequafy first."}\n'
    else
        echo "Error: Not on a feature branch. Run /spequafy first." >&2
    fi
    exit 1
fi
...
if [[ ! -f "$SPEC_FILE" ]]; then
    if $JSON_MODE; then
        printf '{"error":"Specification file not found. Run /spequafy first."}\n'
    else
        echo "Error: Specification file not found at $SPEC_FILE. Run /spequafy first." >&2
    fi
    exit 1
fi
